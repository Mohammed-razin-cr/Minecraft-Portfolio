        ...
        <div className="border-2 border-[#373737] bg-[#C6C6C6] p-2">
          {NAV_ITEMS.map((item) => (
            <motion.button
              ...
              className={`mb-2 flex w-full items-center gap-2 border-2 px-3 py-2 text-left text-sm uppercase tracking-wider transition-colors last:mb-0 ${activeSection === item.id
                ? "border-[#5D8C3C] bg-[#5D8C3C] text-white shadow-lg"
                : "border-[#373737] bg-[#8B8B8B] text-[#373737] hover:bg-[#9B9B9B]"
                }`}
              style={{ willChange: activeSection === item.id ? 'transform' : 'auto' }}
            >
              <span>{item.icon}</span>
          ...
          >
            {NAV_ITEMS.map((item, index) => (
              <motion.button
                ...
                onClick={() => scrollToSection(item.id)}
                className={`mb-2 flex w-full items-center gap-2 sm:gap-3 border-2 px-2 sm:px-4 py-2 sm:py-3 text-xs sm:text-sm uppercase tracking-wider transition-colors ${activeSection === item.id
                  ? "border-[#5D8C3C] bg-[#5D8C3C] text-white shadow-lg"
